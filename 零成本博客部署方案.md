...
## 🚀 快速开始（5步速览）
1. **创建仓库**：用 Hexo 初始化博客（本地一次性或在 Codespaces 中完成），推送到 GitHub。
2. **接入自动部署**：在仓库 `.github/workflows/` 下添加 GitHub Actions 脚本，实现自动构建 + 发布到 `gh-pages` 分支。
3. **开启双平台托管**：在仓库设置中启用 GitHub Pages，同时将仓库导入 Vercel，构建命令 `hexo generate`，输出目录 `public`。
4. **配置域名与 CDN**：DNS 使用智能解析，国内流量指向腾讯云 CDN 的 CNAME，CDN 回源 GitHub Pages；国外流量指向 `cname.vercel-dns.com`。
5. **开始写作**：通过浏览器（github.dev）、Codespaces 或本地 Hexo 写文章，提交即自动部署，1-2 分钟上线。
...
### 7. 示例配置文件
仓库中提供了常用的模版文件，复制后按需调整即可：
- `示例配置文件/hexo-config-example.yml`
- `示例配置文件/github-actions-deploy-example.yml`
- `示例配置文件/vercel-config-example.json`
